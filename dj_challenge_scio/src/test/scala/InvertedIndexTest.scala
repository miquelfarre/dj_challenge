  ...
  it should "check inverted index" in {
    runWithContext { sc =>
      runWithContext { sc =>
        ...
        val dictionary: SCollection[String] = sc.parallelize(pairWordWordIdSeq)
        val fileNameAndLines: SCollection[(String, String)] =
          sc.parallelize(
            Seq(("1", "tremp talarn salas tremp"), ("2", "tremp pobla guardia")))
        val invertedIndexSCollection = BuildInvertedIndex.invertedIndex(fileNameAndLines, dictionary)
